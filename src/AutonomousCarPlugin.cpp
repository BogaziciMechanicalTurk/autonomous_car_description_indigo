...
namespace gazebo {
  ...
  void AutonomousCarPlugin::onTwistCommand(const geometry_msgs::TwistConstPtr& message) {
    ...
    this->dataPointer->lastMessageTime = this->dataPointer->world->GetSimTime();
  ...
  void AutonomousCarPlugin::Update() {
    ...
    std::lock_guard<std::mutex> lock(this->dataPointer->mutex);
    common::Time currentTime = this->dataPointer->world->GetSimTime();
    ...
    dataPointer->frontLeftSteeringAngle = dataPointer->frontLeftWheelSteeringJoint->GetAngle(0).Radian();
    dataPointer->frontRightSteeringAngle = dataPointer->frontRightWheelSteeringJoint->GetAngle(0).Radian();
